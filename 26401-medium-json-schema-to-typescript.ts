...
type GetArray<T> = T extends { type: 'array', items: infer A } ? JSONSchema2TS<A>[] : unknown[]

type GetObject<T> =
  T extends { properties: infer Prop, required: infer R extends string[]}
    ? MergeInsertions<{
        [P in keyof Prop as P extends R[number] ? P : never]-?: JSONSchema2TS<Prop[P]>
      } & {
        [P in keyof Prop as P extends R[number] ? never: P]?: JSONSchema2TS<Prop[P]>
      }>
    : T extends { properties: infer Prop }
      ? {
        [P in keyof Prop]?: JSONSchema2TS<Prop[P]>
        }
...
type JSONSchema2TS<T> =
    T extends { type: 'object' }
      ...
      : T extends { type: 'array' }
        ? GetArray<T>
        : T extends { enum: infer E extends any[] }
          ? E[number]
          : GetBaseType<T>
